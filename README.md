...
- resolved guest IPv4 addresses that are unallocated and, for CloudStack-managed IPAM, inside a configured guest IP range; a source NIC whose IP is unavailable may instead be emitted as immutable `ip:null` plus `ip_override_required:true` and requires one canonical `--nic-ip netN=IPv4` operator binding at execution time; an exact live ROOT/admin L2 VLAN may instead use explicitly frozen external IPAM when its name, VLAN, zone, domain, state and deployability all match;
